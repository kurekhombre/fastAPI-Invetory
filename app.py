...
from models import supplier_pydantic, supplier_pydanticIn, Supplier, product_pydatnic, product_pydatnicIn, Product
...
@app.get('/')
def index():
    return {"message": "Check /docs endpoint"}
...
@app.post('/supplier/{supplier_id}')
async def add_product(supplier_id: int, product_details: product_pydatnicIn):
    supplier = await Supplier.get(id=supplier_id)
    ...
    response = await product_pydatnic.from_tortoise_orm(product_obj)
    return {"status": "ok", "data": response}


@app.get('/product')
async def all_products():
    response = await product_pydatnic.from_queryset(Product.all())
    return {"status": "ok", "data": response}


@app.get('/product/{id}')
async def specific_product(id: int):
    response = await product_pydatnic.from_queryset_single(Product.get(id=id))
    return {"status": "ok", "data": response}


@app.put('/product/{id}')
async def update_product(id: int, update_info: product_pydatnicIn):
    ...
    await product.save()
    response = await product_pydatnic.from_tortoise_orm(product)
    return {"status": "ok", "data": response}
